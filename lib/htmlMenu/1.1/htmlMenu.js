...
class HTMLMenu {
  ...
  _getShowMenuCallback( name, paramInfo ) {
    ...
    const element = menuInfo.element;
    ...
    const _showMenu = ( obj )=> {
      ...
      const computeItemVisibility = ( menuInfo )=> {
        for( const menuItem of menuInfo.itemList ) {
          ...
          if( !menuItem.if || menuItem.if( o ) ) {
            // Make the menu item visible if the 'if' is satistied
            //menuItem.element.classList.add( 'show-mdd-menu' );
      ...
      if( true || isMenuItemVisible ) {
        // Make context menu visible (container)
        element.classList.add( 'show-mdd-menu' );
        // Compute menu position
        const mouseXY = paramInfo.getMenuClick();
        this._computeRootMenuMouseClickXY( mouseXY, element );
        // Set menu position
        element.style.left = mouseXY.x + 5 + "px";
  ...
  _getHideMenuCallback( name, paramInfo ) {
    ...
    const _hideMenu = ()=> {
      // Compute visibility for all items
      const makeItemInvisible = ( menuInfo )=> {
        for( const menuItem of menuInfo.itemList ) {
          // Hide item
          menuItem.element.classList.remove( 'hide-mdd-menu' );
          if( menuItem.itemList ) {
            // Hide contained items
            makeItemInvisible( menuItem );
          }
        }
      };
      // Hide context menu items
      makeItemInvisible( menuInfo );
      // Hide context menu
      element.classList.remove( 'show-mdd-menu' );

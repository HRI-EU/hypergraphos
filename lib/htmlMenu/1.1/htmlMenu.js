...
class HTMLMenu {
  ...
  _getMouseItemOnMouseOver( menuItem ) {
    const onMouseOver = ( event )=> {
      ...
      /*
      // Get list of HTML siblings
      const parentEl = el.parentElement;
      // Hide all sub menu at this current level
      this._setElementChildVisibility( parentEl, 'hidden', 1, Infinity );
      */
  ...
  _makeMenuHidden( menuInfo, parentEl ) {
    ...
    */
    ...
    // Hide all sub menu at this current level
    //this._setElementChildVisibility( parentEl, 'hidden', 0, Infinity );
  ...
  _setElementChildVisibility( el, status, deepthSkip, depthApply ) {
    ...
    if( el.childElementCount ) {
      ...
      for( const el of childList ) {
        if( deepthSkip == 0 ) {  // If  Skip is over -> apply visibility
          ...
          el.classList.remove( 'hide-mdd-menu' );
        }

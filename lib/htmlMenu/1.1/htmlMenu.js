...
class HTMLMenu2 {
  constructor( parentId,) {
    ...
    if( canvasId ) {
      ...
      this.canvasEl.addEventListener( "contextmenu", 
            (e)=>{ 
              console.log( 'Show Context '+JSON.stringify({x:e.clientX, y:e.clientY}))
            }, false);
    }*/
  }
  add( menuInfoList ) {
  ...
  addMenuItemParams( contextName, params ) {
    // Add a new context with item parameters
    this.menuItemParamContextList[contextName] = params;
    // Switch to the new context
    this.switcheMenuItemParams( contextName );
  }
  switcheMenuItemParams( contextName ) {
    // Get params
    const params = this.menuItemParamContextList[contextName];
    // Switch to context
    this.menuItemParamD = params;
  }
  getGoJSMenu( name ) {
    // Get GoJS diagram
    const diagram = this.menuItemParamContextList['gojs'].diagram;
    // Create GoJS context menu wrapper object
    const contextMenu = $(go.HTMLInfo, {
      show: this._getShowMenuCallback( name, diagram ),
      hide: this._getHideMenuCallback( name, diagram ),
    });
  ...
  _newRootMenu( menuInfo, name ) {
    if( menuInfo.itemList ) {
      ...
      // Create full menu
      this._createMenu( menuInfo, parent, name );
      // Add menu to class instance
      this.menuList[name] = menuInfo;
    }
  }
  _createMenu( menuItem, parent, name ) {
    // Create HTML menu container
    this._createMenuContainer( menuItem, parent, name );
    // Create menu items
    this._createMenuItems( menuItem.itemList, menuItem.element );
  }
  _createMenuItems( menuItemList, parent ) {
    // Add all items in the menu
    for( const menuItem of menuItemList ) {
      // Create HTML item element
      if( menuItem.fontIcon ) {
        this._createItemFontIcon( menuItem, parent );
      } else if( menuItem.label ) {
        this._createItemLabel( menuItem, parent );
      } else if( menuItem.separator ) {
        this._createItemSeparator( menuItem, parent );
      }
      ...
      if( menuItem.itemList ) {
        // Create full menu
        this._createMenu( menuItem, parent );
  ...
  _createMenuContainer( menuItem, parent, name ) {
  ...
  _createItemFontIcon( menuItem, parent ) {
    menuItem.type = 'item';
  ...
  _createItemLabel( menuItem, parent ) {
  ...
  _createItemSeparator( menuItem , parent) {
    menuItem.type = 'item';
  ...
  _getShowMenuCallback( name, diagram ) {
    // Get context menu
    ...
    const _showMenu = ( name )=> {
      el.classList.add( 'show-mdd-menu' );
      //el.style.visibility = 'visible';
      const mousePt = diagram.lastInput.viewPoint;
      this._computeRootMenuMouseClickXY( mousePt, el );
      el.style.left = mousePt.x + 5 + "px";
      el.style.top = mousePt.y + "px";

      // Optional: Use a `window` click listener with event capture to
      //           remove the context menu if the user clicks elsewhere on the page
      window.addEventListener( 'click', this.hideCX.bind(this), true );
    }
    return( _showMenu );
  }
  _getHideMenuCallback( name, diagram ) {
    // Get context menu
    const menuInfo = this.get( name );
    // Get constext menu HTML element
    const el = menuInfo.element;
    ...
    const _hideMenu = ()=> {
      el.classList.remove( 'show-mdd-menu' );

      // Optional: Use a `window` click listener with event capture to
      //           remove the context menu if the user clicks elsewhere on the page
      window.removeEventListener( 'click', this.hideCX.bind(this), true );
    }
    return( _hideMenu );
  }
  hideCX() {
  ...
  _computeRootMenuMouseClickXY( mousePoint, element ) {

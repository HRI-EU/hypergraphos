class DataFlowEngine {
  ...
  getPropertyNameList( nodeData ) {
    let result = [];
    if( nodeData.rows ) {
      nodeData.in.filter( (r)=> result.push( r.name ) );
  ...
  getProperty( nodeData, name, defaultValue ) {
    ...
    if( nodeData.rows ) {
      nodeData.rows.find( (r)=> { if( r.name == name ) result = r.value });
    }
    return( result ); 
  ...
  getPropertyList( nodeData ) {
    let result = {};
    if( nodeData.rows ) {
      nodeData.in.filter( (r)=> result[r.name] = r.value );
      // for( const row of nodeData.rows ) {
  ...
  isProperty( nodeData, name ) {
    let result = false;
    if( nodeData.rows ) {
      nodeData.rows.find( (r)=> { if( r.name == name ) result = true });
    }
    return( result );
  }
  _getPortNameRef( nodeData, inOut, name ) {
    let result = null;
    const portList = nodeData[inOut];
    if( portList ) {
      for( const port of portList ) {
        ...
        let ref = null;
        if( idx >= 0 ) {
          const pname = portId.substring( 0, idx );
          ref = portId.substring( idx+1 );
          return( { name: pname, ref } );
        } else if( portId == name ) {
          return( { name: portId, ref: null } );
        }
      }
    }
    return( result );
  }
  _isPortName( nodeData, inOut, name ) {
    let result = false;
    const portList = nodeData[inOut];
    if( portList ) {
      for( const port of portList ) {
  ...
  isInput( nodeData, name ) {
    let result = false;
    if( nodeData.in ) {
      nodeData.in.find( (i)=> { if( i.portId == name ) result = true });
    }
    return( result );
  }
  isOutput( nodeData, name ) {
    let result = false;
    if( nodeData.out ) {
      nodeData.out.find( (o)=> { if( o.portId == name ) result = true });
    }
    return (result);
  }
  ...
  getOutputNameList(nodeData) {
    ...
    if (nodeData.out) {
      ...
      // for (const outInfo of nodeData.out) {
      //   result.push(outInfo.portId);
      // }
    }
    return (result);
  }
  ...
  getInput(nodeData, name, defaultValue) {
    ...
    const refInfo = parseRefValue( nameRef );

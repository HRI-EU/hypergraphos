...
class LineBlockGenerator {
    ...
    while( idx < templateLen ) {
      ...
      if( lineMatch && lineMatch[1] ) {
        ...
        if( startBlockName.startsWith( this.conf.beginTag ) ) {
          ...
          let dataValue = this._getKeyValue( startName, dataReader );
  ...
  extractLineBlock( name, trimIndentation, prefix, postfix ) {
    let idx = 0;
  ...
  _appendToOutput( name, indent, dataReader ) {
    ...
    let value = this._getKeyValue( name, dataReader );

...
function Engine_getProperty( nodeData, name, defaultValue ) {
  const value = appData.me.getProperty( modelId, nodeData, name );
  if( value != undefined ) {
    return( value );
  } else {
    return( defaultValue );
...
function Engine_clearInputs( nodeData, nameList ) {
  ...
  for( const name in inState._in ) {
    inState._in[name].value = null;
    inState._in[name].isNew = false;
  }
}
...
// DSL Functions
//------------------------------
function DataFlowDSL_Template( nodeData, name, value ) {
  // Update input values
  ...
  if( inState.isCompute ) {
    ...
    if( templateStr ) {
      //const inState = Engine_get( nodeData, 'inState', {} );
      const outValue = Engine_applyTemplate( templateStr, inState._in );
      Engine_fireOutput( nodeData, 'out', outValue );
    }
  }
}
function DataFlowDSL_Concatenate( nodeData, name, value ) {
...
function DataFlowDSL_Data( nodeData, name, value ) {
  let outValue = value;
  ...
  if( nodeData.category == 'DataFlow_Data' ) {

...
function getTableRowValue( props_, name ) {
  let result = '';
  if( props_ ) {
    for( const item of props_ ) {
      if( item.name == name) {
        result = item.value;
        break;
      }
...
class MironModel {
  ...
  Loop_Begin_Miron_Templates() {
    ...
    const fakeTemplateData = { props_: [ { name: fakeTemplate } ] };
  ...
  Define_FileName() {
    ...
    const d = this.g.getDataIf( this.node, 
                                (n,d)=> ( d.category == 'Hierarchy_GraphInfo') &&
                                        ( d.label == 'Graph Info' ) );
    if( d ) {
      let path = getTableRowValue( d.props_, 'Path' );
      let name = getTableRowValue( d.props_, 'Name' );
  ...
  Begin_Header_Info() {
    ...
    const d = this.g.getDataIf( this.node, 
                                (n,d)=> ( d.category == 'Hierarchy_GraphInfo') &&
                                        ( d.label == 'Graph Info' ) );
    if( d ) {
      let version = getTableRowValue( d.props_, 'Version' );
      let date = getTableRowValue( d.props_, 'Date' );
      result.push( [ version ] );
      result.push( [ date ] );
    }
    return( result );

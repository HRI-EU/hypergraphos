  ...
  */
  ...
  function onPropertyValueEdited(tb, oldstr, newstr)  {
    ...
    if( node ) {
      ...
      // newstr is == or != to the dataNodeTemplate.props_[rowIndex].value
  ...
  function getMenuItemText( o, target ) {
    ...
    const rowIndex = o.d.obj.panel.itemIndex; // row index to modify in the node.data.props_
    const node = it.first();
    if( node ) {
      const data = node.data;
...
console.log( 'Button Status: '+obj.data.checked );`;
  ...
  var customNodeSelection = {};
    ...
    //     )  // end Adornment
    // };
  var customLinkSelection = {};
    // { // TODO: Make a better selection
    //   selectionAdornmentTemplate:
    //     $(go.Adornment,
    //       $(go.Shape,
    //         { isPanelMain: true, stroke: "red", strokeWidth: 8 }),
    //       $(go.Shape,
    //         { toArrow: "Standard", fill: "red", stroke: null, scale: 2.5 })
    //     )  // end Adornment
    // };
  ...
  var dsl_Component = (param)=> {
    ...
    function runGraphFile( event, obj ) {
      const executeButtonFunc = function( event, obj, nodeData, fileContent ) {
      ...
      getNodeDataFileContent( nodeData, (fileContent)=> {
        executeButtonFunc( event, obj, nodeData, fileContent );
      });
    };
        ...
        new go.Binding("font", "font").makeTwoWay(),
        {
          ...
          $(go.Adornment, "Auto",
            ...
            $(go.TextBlock, { margin: 4 },  // margin to give room for the drop shadow
              // TextBlock.text is bound to Node.data.tooltip
              new go.Binding("text", "tooltip"),
              new go.Binding("text", "unitTooltip"))
          ),  // end of Adornment
        }
      );
    }
    ...
    if( param.hasFunctionButtons ) {
      ...
      $(go.Panel, "Vertical",
        ...
        new go.Binding("itemArray", "buttons_"),
        {
          row: 0, column: 0,
          itemTemplate:
          $(buttonType, ( param.isCheckBoxes? "checked": {} ),
            { 
            ...
            new go.Binding("minSize", "buttonMinSize", go.Size.parse).makeTwoWay(go.Size.stringify),
            $(go.TextBlock,
              {
                //text: "Click me!",
                margin: param.buttonMargin,
              ...
              new go.Binding("font", "buttonFont")
            )
          ) // Button
        } 
      ); // Vertical Panel 
  ...
  var dsl_BasicNode = (param)=> {
    ...
    // This allow to sync the width of text to the node width allowing wraping
    let wrapTagBinding = new go.Binding("width", "size", function(size) {
      var values = size.split(" ");
      var width = parseFloat(values[0]);
      return width;
    });
  ...
  var dsl_BasicLink = (param)=> {
    ...
    // LINK LABEL SHAPE
    param.hasLabelShape = ( param.hasLabelShape !== undefined? param.hasLabelShape: false );
    param.labelShape = ( param.labelShape? param.labelShape: "Diamond" );
    param.labelShapeFill = ( param.labelShapeFill? param.labelShapeFill: "	#FFFF00BB" );
    param.labelShapeStroke = ( param.labelShapeStroke? param.labelShapeStroke: "gray" );
    // LINK LABEL
    param.hasLabel = ( param.hasLabel !== undefined? param.hasLabel: false );
    param.label = ( param.label !== undefined? param.label: '' );
    param.labelStroke = ( param.labelStroke? param.labelStroke: "Black" );
    param.isLabelEditable = ( param.isLabelEditable !== undefined? param.isLabelEditable: true );
    param.isLabelMultiline = ( param.isLabelMultiline !== undefined? param.isLabelMultiline: true );
    param.labelTextAlign = ( param.labelTextAlign? param.labelTextAlign: "center" );
    param.labelFont = ( param.labelFont? param.labelFont: "18px sans-serif" );
    param.labelMargin = ( param.labelMargin? param.labelMargin: 3 );
    param.labelVerticalAlignment = ( param.labelVerticalAlignment? param.labelVerticalAlignment: "center" );
    param.labelHorizontalAlignment = ( param.labelHorizontalAlignment? param.labelHorizontalAlignment: "center" );
    // FROM END
    ...
    let labelShape = {};
    if( param.hasLabelShape ) {
      labelShape = $(go.Shape, 
        { 
          name: "LABEL SHAPE",
          figure: param.labelShape, 
          fill: param.labelShapeFill, 
          stroke: param.labelShapeStroke 
        }
      );
    }
    let label = {};
    if( param.hasLabel ) {
      label = $(go.Panel, "Auto",  // this whole Panel is a link label
        labelShape,
        $(go.TextBlock, 
          { 
            name: "LABEL",
            minSize: new go.Size(NaN, 20), 
            text: param.label,
            background: param.labelShapeFill, 
            stroke: param.labelStroke,
            editable: param.isLabelEditable,
            isMultiline: param.isLabelMultiline,
            textAlign: param.labelTextAlign,
  ...
  function turnOnVisibitityOfButtons(evt,diagram) {
    diagram.startTransaction( "turn on visibility of part on deselect" );
    const selection = evt.subject;
    // Loop over selected objects
    selection.each(
      function(value) {
        const data = value.data;
        if( data && (data.props_ || data.in_ || data.out_ || data.tag ) ) {
          const objList = findAllObject(value,"HIDE ON DESELECT");
          objList.each(
            function(v) {
    ...
    diagram.commitTransaction( "turn on visibility of part on deselect" );
  };
  function turnOffVisibitityOfButtons(evt,diagram) {
    diagram.startTransaction( "turn off visibility of part on deselect" );
    const selection = evt.subject;
    // Loop over selected objects
    selection.each(
      function(value) {

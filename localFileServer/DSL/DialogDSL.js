...
/*
   DSL for Graph Dialog
    ...
    - By Frankonello
*/
function DialogDSL_includeList() {
  return(['/fileServer/System/Includes/DSLInclude.js']);
}
function DialogDSL_getDSL( g ) {
  ...
  */
  ...
  function findPortSelectedItems( node , portType ) {
    // Change done to globally add names to portId
    //(\\"portId\\":\s*\\")(\w+)(\\")(,\\"[^n])
    //$1$2$3,\"name\":\"$2\"$4
    const items = [];
    let maxId = 0;
    const portSide = ( portType == 'in'? 'INPORT': 'OUTPORT' );
    const portPanel = node.findObject( portSide );
    if( portPanel ) {
      // Iterate on all port items of portSide (IN/OUT) the component
      for( let it1 = portPanel.elements; it1.next(); ) {
        const panelItem = it1.value;
        
        // Update maxId
        const portData = panelItem.data; // { portId, name }
        const currIdIdx = parseInt( portData.portId );
        if( currIdIdx > maxId ) {
          maxId = currIdIdx;
        }
  ...
  function addPort( e, obj, portType ) {
    const node = obj.part;
    if( node ) {
      const [ items, maxId ] = findPortSelectedItems( node, portType );
      const diagram = node.diagram;
      diagram.startTransaction( `Add ${portType} port` );

      let itemIndex = -1;
      let nodeData;
      let itemArray;
      let portId = `${maxId+1}${portType}`;
      let name = `${portType}${maxId+1}`;
      if( items.length > 0 ) {  // if there are any selected items, delete them
        const selectIndex = 0;
        const panel = items[selectIndex];
        nodeData = panel.part.data;
        itemArray = ( portType == 'in'? nodeData.in_: nodeData.out_ );
        const itemData = panel.data;
        itemIndex = itemArray.indexOf( itemData );
      } else {
        nodeData = node.data;
      ...
      diagram.model.insertArrayItem( itemArray, itemIndex, { portId, name } );

      diagram.commitTransaction( `Add ${portType} port` );
    }
  }
  ...
  function deletePort( e, obj, portType ) {
    const node = obj.part;
    if( node ) {
      const [ items ] = findPortSelectedItems( node, portType );
      if( items.length > 0 ) {  // if there are any selected items, delete them
        const diagram = node.diagram;
        diagram.startTransaction( `Delete ${portType} port` );
        for( let i = 0; i < items.length; i++ ) {
          const panel = items[i];
          const nodeData = panel.part.data;
          const itemArray = ( portType == 'in'? nodeData.in_: nodeData.out_ );
          const itemData = panel.data;
          const itemIndex = itemArray.indexOf( itemData );
          diagram.model.removeArrayItem( itemArray, itemIndex );
        }
        diagram.commitTransaction( `Delete ${portType} port` );
      }
    }
  }
        ...
        $(go.Shape, "Rectangle",
          {
            ...
            name: "BODY",
            fill: "white", 
            portId: 'ConditionRef', name:'ConditionRef', 
            cursor: "pointer",  
          ...
          new go.Binding("fill", "modality", fillFunc),
      ...
      $(go.Panel, "Auto",
        {
          ...
          portId: 'Condition', name:'Condition', 
          cursor: "pointer",  
          // allow outgoing links
          fromLinkable: true, 
          fromLinkableSelfNode: false, 
        ...
        $(go.Shape, "Rectangle",
          {
            ...
            maxSize: new go.Size(NaN, 40),
            fill: "white", 
            portId: 'ActionRef', name:'ActionRef', 
            cursor: "pointer",  
            // allow outgoing links
            fromLinkable: true, 
            fromLinkableSelfNode: false, 
   ...
   if( diagram ) {
    diagram.addDiagramListener( "LinkDrawn",
      function(evt) {
        const linkCategory = {
          ConditionRef: {
            ConditionRef: "Dialog_Reference",
            And:          "x",
            InhibitAnd:   "x",
            Rule:         "x",
            Action:       "x",
            ActionRef:    "Dialog_Reference",
            Grammar:      "x",
            RuleComment:  "x",
            PortOut:      "Dialog_Component",
          },
          Condition: {
            ConditionRef: "x",
            And:          "Dialog_Condition",
            InhibitAnd:   "Dialog_Condition",
            Rule:         "Dialog_Condition",
            Action:       "x",
            ActionRef:    "x",
            Grammar:      "x",
            RuleComment:  "x",
          ...
          Rule: {
            ConditionRef: "x",
            And:          "Dialog_Rule",
            InhibitAnd:   "Dialog_Rule",
            Rule:         "Dialog_Rule",
            Action:       "Dialog_Action",
            ActionRef:    "x",
            Grammar:      "x",
            RuleComment:  "Dialog_Comment",
            PortOut:      "x",
          },
          ActionRef: {
            ConditionRef: "Dialog_Reference",
            ...
            InhibitAnd:   "x",
            Rule:         "x",
            Action:       "Dialog_Grammar",
            ActionRef:    "Dialog_Reference",
            Grammar:      "Dialog_Grammar",
            RuleComment:  "x",
            PortOut:      "Dialog_Component",
          },
          PortIn: {
            ConditionRef: "Dialog_Component",
            And:          "x",
            InhibitAnd:   "x",
            Rule:         "x",
            Action:       "x",
            ActionRef:    "Dialog_Component",
            Grammar:      "x",
            RuleComment:  "x",
            PortOut:      "x",
          }
  ...
  const dsl = {
    ...
    templateNodeList: [
      { category: 'Dialog_WMCondition',       template: dsl_Condition, },
      { category: 'Dialog_MironCondition',    template: dsl_Condition, },
      { category: 'Dialog_SlotCondition',     template: dsl_Condition, },
      { category: 'Dialog_MironAction',       template: dsl_Action, },
      { category: 'Dialog_WMAction',          template: dsl_Action, },
      { category: 'Dialog_And',               template: dsl_BasicNode, param: {hasTag: false, hasType:false, maxSize: new go.Size(40,40),  figure: "circle", fill: "green",      label: "&",     portId: "And",         labelTextAlign: "center", isFromLinkable: true,  isToLinkable: true, labelStroke: "white", isLabelEditable: false,} },
      { category: 'Dialog_InhibitAnd',        template: dsl_BasicNode, param: {hasTag: false, hasType:false, maxSize: new go.Size(40,40),  figure: "circle", fill: "IndianRed",  label: "&",     portId: "InhibitAnd",  labelTextAlign: "center", isFromLinkable: true,  isToLinkable: true, labelStroke: "white", isLabelEditable: false,} },
      { category: 'Dialog_Rule',              template: dsl_BasicNode, param: {hasTag: false, hasType:false, maxSize: new go.Size(80,80),  figure: "circle", fill: "peru",       label: "@key",  portId: "Rule",        labelTextAlign: "center", isFromLinkable: true,  isToLinkable: true, labelStroke: "white"} },
      { category: 'Dialog_BigComment',        template: dsl_BasicNode, param: {hasTag: false, hasType:false, minSize: new go.Size(300,80), labelFont: "bold 32px sans-serif",                                                labelTextAlign: "center", isFromLinkable: false, isToLinkable: false, labelStroke: "darkgray",} },
      { category: 'Dialog_RuleComment',       template: dsl_BasicNode, param: {hasTag: false, hasType:false, minSize: new go.Size(100,40), labelFont: "bold 20px sans-serif",                         portId: "RuleComment", labelTextAlign: "left",   isFromLinkable: false, isToLinkable: true,  toSpot: go.Spot.Left, toMaxLinks: 1, fill: "LightGray", labelMargin: 10,} },
      { category: 'Dialog_Slot',              template: dsl_Component, param: {hasInputs: false, hasOutputs: false, hasFunctionButtons: false, hasTag: true,  hasType: true,  hasValue: true,  hasUnit: false,  figure: "SquareUShape",  fill: "Gold", isLabelEditable: false, keyStroke: "red", valueStroke: "blue", portId: "Grammar",  isFromLinkable: false, isToLinkable: true, toSpot: go.Spot.Left, toMaxLinks: 1,} },
      { category: 'Dialog_DataSlot',          template: dsl_Component, param: {hasInputs: false, hasOutputs: false, hasFunctionButtons: false, hasTag: true,  hasType: true,  hasValue: true,  hasUnit: false,  figure: "UShapeInternalArrowOut",  fill: "GoldenRod", isLabelEditable: false, keyStroke: "red", valueStroke: "blue", portId: "Grammar",  isFromLinkable: false, isToLinkable: true, toSpot: go.Spot.Left, toMaxLinks: 1,} },
      { category: 'Dialog_MironTemplate',     template: dsl_Component, param: {hasInputs: false, hasOutputs: false, hasFunctionButtons: false, hasTag: true,  hasType: true,  hasValue: false,  hasUnit: false,  figure: "RoundedRectangle",  fill: "LemonChiffon", isLabelEditable: false, keyStroke: "blue", portId: "Grammar",  isFromLinkable: false, isToLinkable: true, toSpot: go.Spot.Left, toMaxLinks: 1,} },
      { category: 'Dialog_Grammar',           template: dsl_Component, param: {hasInputs: false, hasOutputs: false, hasFunctionButtons: false, hasTag: true,  hasType: true,  hasValue: false,  hasUnit: false,  figure: "BendedLeftRight",  fill: "LemonChiffon", isLabelEditable: true, keyStroke: "blue", portId: "",  isFromLinkable: false, isToLinkable: false,} },
      { category: 'Dialog_Component',         template: dsl_Component, param: {hasTag: false, hasType:false, figure: "Rectangle",              fill: "lightcyan",    isLabelEditable: true, portId: "", isFromLinkable: false, isToLinkable: false, hasProperties: false} }, 
      { category: 'Dialog_PortIn',            template: dsl_Component, param: {hasTag: false, hasType:false, figure: "Rectangle",              fill: "lightcyan",    isLabelEditable: true, portId: "PortIn", isFromLinkable: true, isToLinkable: false, fromSpot: go.Spot.Right, hasProperties: false, isResizable: true, isInputLinkable: true, isInputEditable: false, hasOutputs:false, canAddInput: false } }, 
      { category: 'Dialog_PortOut',           template: dsl_Component, param: {hasTag: false, hasType:false, figure: "Rectangle",              fill: "lightcyan",    isLabelEditable: true, portId: "PortOut", isFromLinkable: false, isToLinkable: true, toSpot: go.Spot.Left, hasProperties: false, isResizable: true, isOutputLinkable: true, isOutputEditable: false, hasInputs:false, canAddOutput: false } }, 
    ],
    // Define node palette
    dataNodeList: [
      {
        mironName: 'Test memory state',
        category: 'Dialog_WMCondition',
        modality: "wm",
        type: "is wm activated?",
      },
      {
        mironName: 'Miron is recognized',
        category: 'Dialog_MironCondition',
        modality: "speech",
      ...
      },
      {
        ...
        color: 'Gold',
        category: 'Dialog_SlotCondition',
        modality: "emptySlot",
        type: "is active?",
      },
      {
        category: 'Dialog_InhibitAnd',
      },
      {
        category: 'Dialog_And',
      },
      {
        category: 'Dialog_Rule',
      },
      {
        mironName: 'Action',
        category: 'Dialog_MironAction',
        modality: "speech",
        type: "do outer miron!",
      },
      {
      ...
      },
      {
        ...
        category: 'Dialog_WMAction',
        modality: "wm",
        type: "activate wm!",
      },
      {
        label: 'Rule comment',
        category: 'Dialog_RuleComment',
        size: "300 40",
      },
      {
        label: 'Big comment',
        category: 'Dialog_BigComment',
        size: "300 70",
      },
      {
        label: 'Slot',
        category: 'Dialog_Slot',
        size: "200 40",
        props_: [
    ...
    templateLinkList: [
      { category: 'Dialog_Rule',      template: dsl_BasicLink, param: {stroke:"black", toArrow: "standard", toScale: 2, strokeWidth: 4,toShortLength:8}},
      { category: 'Dialog_Condition', template: dsl_BasicLink, param: {stroke:"black", toArrow: "diamond", toScale: 2, strokeWidth: 4,toShortLength:8}},
      { category: 'Dialog_Inhibit',   template: dsl_BasicLink, param: {stroke:"red", toArrow: "circle", toScale: 2, strokeWidth: 4,}},
      { category: 'Dialog_Comment',   template: dsl_BasicLink, param: {stroke:"black",  strokeDashArray: [2, 2], strokeWidth: 4,}},
      { category: 'Dialog_Action',    template: dsl_BasicLink, param: {stroke:"black",  strokeWidth: 4,}},
      { category: 'Dialog_Grammar',   template: dsl_BasicLink, param: {stroke:"DarkOrange", strokeWidth: 4,}},
      { category: 'Dialog_Reference', template: dsl_BasicLink, param: {stroke:"GreenYellow",strokeWidth: 4, strokeDashArray: [5, 5]}},
      { category: 'Dialog_Component', template: dsl_BasicLink, param: {stroke:"lightslategray",  strokeWidth: 4,}},
    ],
    // Define link palette
    dataLinkList: [
      {
      ...
      {
        category: 'Dialog_Condition',
        fromPort: "",
        toPort: "",
      },
      {
        category: 'Dialog_Inhibit',
        fromPort: "",
        toPort: "",
      },
      {
        category: 'Dialog_Comment',
        fromPort: "",
        toPort: "",
      },
      {
        category: 'Dialog_Action',
        fromPort: "",
        toPort: "",
      },
      { 

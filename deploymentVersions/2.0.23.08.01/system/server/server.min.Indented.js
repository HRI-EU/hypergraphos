...
/* MAP:
Espresso             -> e1 (except last occurrence: './Espresso' )
static(              -> s2(
addGET(              -> a3(
getRequestHandler    -> g4
dispatchHandler      -> d5
addPOST(             -> a6(
_sortHandlers(       -> s7(
_handleRequest(      -> h8(
_methodGETHandler(   -> m9(
_methodPOSTHandler(  -> m10(
GETStatus            -> g11
GETFileStatus        -> g12
recomputeURL(        -> r13(
ExecuteScript        -> e15
 ...
 */
   (function(){
    ...
    class e1 {
      ...
      a3( virtualPath, fileServer, onGet ) {
        const method = 'GET';
        const g4 = (req, resp) => {
          req.addListener( 'end', ()=>{ 
            if( onGet ) {
        ...
        this.d5[method].push( handler );
        this.s7( method );
      }
      a6( virtualPath, fileServer ) {
        const method = 'POST';
        const g4 = (req, resp) => {
          let body = '';
      ...
      g4() {
        return( (req, resp)=> this.h8(req,resp) );
      }
      h8( request, response ) {
        try {
          switch( request.method ) {
            case 'GET': 
              this.m9( request, response );
              break;
            case 'POST':
              this.m10( request, response );
              break;
          }
        } catch( error ) {
          console.log( error );
      ...
      m9( request, response ) {
        ...
        for( const handler of this.d5[method] ) {
    ...
    class g12 {
      ...
      serve( request, response ) {
        let filePath = r13( request.url, this.virtualPath, this.realPath );
        ...
        try {
          ...
          if( fileInfo && fileInfo.url ) {
            ...
            if( fileInfo.url.startsWith( this.virtualPath ) ) {
              // Get updated url
              const filePathName = r13( fileInfo.url, this.virtualPath, this.realPath );
              const pathInfo = g17( filePathName );
    ...
    class e15 {
      ...
      serve( request, response ) {
        ...
        let path = r13( request.url, this.virtualPath, this.realPath );
    ...
    var webServer = http.createServer( espresso.g4() );
    
    const onGet = function( request, response ) {
      console.log( 'Loading file: '+request.url );
    }
    // Serve libraries files
    espresso.a3(  '/library',    espresso.s2( config.server.libPath ), onGet );
    // Load files from dataRoot
    espresso.a3(  '/fileServer', espresso.s2( config.server.dataRoot ), onGet );
    // Execute server script
    espresso.a3(  '/executeScript', new e15( config.server.scriptPath ), onGet );
    // Serve client files
    espresso.a3(  '/',           espresso.s2( config.server.clientPath ), onGet );
    // Status logger
    espresso.a3( '/status', new g11() );
    // Status of files (date, size, ...)
    espresso.a3( '/fileStatus', new g12( config.server.dataRoot ) );
    // Save files to dataRoot
    espresso.a6( '/fileServer', new POSTServer( config.server.dataRoot ) );
    // Serve libraries files
    espresso.a3(  '/test',    espresso.s2( config.server.testPath ), onGet );
    ...
    webServer.listen(config.server.webServerPort, function(){
      ...
      console.log( ` Access with user: ${url}?name:'User'` );
    });
    ...
    function r13( url, virtualPath, realPath ) {
